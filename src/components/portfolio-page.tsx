...
const allCategories = ["All", ...projects.map((p) => p.category)];
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <section className="relative py-24 overflow-hidden">
        ...
        >
          ...
          <h1 className="font-display mt-4 text-5xl font-extrabold tracking-tight md:text-6xl">Portfolio</h1>
          <p className="mt-5 mx-auto max-w-2xl text-lg text-muted-foreground">
            Real products built for real businesses. Here's a showcase of what we've shipped across
            web, mobile, enterprise and AI.
          </p>
        </motion.div>
      </section>
      ...
      <section className="sticky top-[72px] z-30 border-b border-border bg-background/80 backdrop-blur-lg">
        <div className="mx-auto max-w-7xl px-6 py-4 flex items-center gap-3 overflow-x-auto scrollbar-none">
          <Filter className="h-4 w-4 text-muted-foreground flex-none" />
          {allCategories.map((cat) => (
            <button
              key={cat}
              onClick={() => setActiveFilter(cat)}
              className={`flex-none rounded-full px-5 py-2 text-xs font-semibold transition-all duration-300 ${
                activeFilter === cat
                  ? "bg-gradient-brand text-primary-foreground shadow-glow"
                  : "border border-border bg-surface hover:bg-surface-elevated text-muted-foreground"
              }`}
            >
              {cat}
            </button>
        ...
        <div className="mx-auto max-w-4xl px-6 text-center">
          <div className="gradient-border-ring relative overflow-hidden rounded-[2rem] bg-gradient-brand p-12 shadow-glow">
            ...
            <div className="relative">
              <h2 className="font-display text-3xl md:text-4xl font-bold text-white">Have a project in mind?</h2>
